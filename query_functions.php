...
function call_image($imgsearch){
    global $conn; 

    $tableName = "reviews";
    $imageId = '3amXYLYLejQnnNw03ePC.jpg';  // Replace with the actual ID of the image you want to fetch

    // Fetch the image data from the database
    $sql = "SELECT image FROM $tableName WHERE id = $imageId";
    ...
    if ($result->num_rows > 0) {
        ...
        // Display the image
        header("Content-type: image/jpg");  // Adjust the content type based on your image format
        echo $row['image_data'];
    } else {
